...
class LastSeenQueryE2ETests(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp_dir = tempfile.TemporaryDirectory(prefix="vision_butler_t15_e2e_last_seen_")
        ...
        for name in [
            ...
            if name == "settings.yaml":
                content["database"]["path"] = str(Path(self.tmp_dir.name) / "last_seen_query.db")
            if name == "access.yaml":
    ...
    def _seed_runtime_data(self) -> None:
        self.client.post(
            ...
            json={
                ...
                "status": "online",
                "trace_id": "t15-e2e-lastseen-heartbeat",
            },
        )
        self.client.post(
            "/device/ingest/event",
            json={
                ...
                "object_name": "package",
                "confidence": 0.94,
                "trace_id": "t15-e2e-lastseen-ingest",
            },
        )
    ...
    def test_last_seen_command(self) -> None:
        response = self.client.post(
            "/telegram/update",
            json=self._build_update(update_id=6201, text="/lastseen package"),
        )

        self.assertEqual(response.status_code, 200)
        payload = response.json()["data"]
        self.assertEqual(payload["status"], "processed")
        self.assertEqual(payload["command"], "lastseen")
