...
class TakeSnapshotCommandE2ETests(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp_dir = tempfile.TemporaryDirectory(prefix="vision_butler_t15_e2e_snapshot_")
        ...
        for name in [
            ...
            if name == "settings.yaml":
                content["database"]["path"] = str(Path(self.tmp_dir.name) / "take_snapshot_command.db")
            if name == "access.yaml":
        ...
        self.client.post(
            ...
            json={
                ...
                "status": "online",
                "trace_id": "t15-e2e-snapshot-heartbeat",
            },
        )
    ...
    def test_take_snapshot_command(self) -> None:
        response = self.client.post(
            "/telegram/update",
            json=self._build_update(update_id=6501, text="/snapshot rk3566-dev-01"),
        )
        ...
        self.assertIn("拍照完成", payload["outbound_messages"][0]["text"])
